...
export default function App() {
  ...
  const handleAuth = async (mode) => {
    ...
    if (mode === "register" && !authForm.email) {
      setAuthError("Please enter your email.");
      return;
    }
    if (mode === "register" && !/\S+@\S+\.\S+/.test(authForm.email)) {
    ...
    try {
      ...
      const res = await api.post(`/api/auth/${mode}`, payload);
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setResetMsg(e.response?.data?.error || "Reset failed. Link may have expired.");
    } finally {
    ...
    return (
      <div style={s.authPage}>
        <div style={s.authCard}>
          ...
          <p style={s.authSub}>Set your new password</p>
          <input style={s.input} placeholder="New password (min 6 chars)"
            type="password" value={resetPass}
            onChange={e => setResetPass(e.target.value)}
            onKeyDown={e => e.key === "Enter" && handleResetPassword()} />
          {resetMsg && (
            <p style={{ ...s.authError, color: resetMsg.includes("successful") ? "#22c55e" : "#ef4444" }}>
              {resetMsg}
            </p>
          )}
          <button style={s.primaryBtn} onClick={handleResetPassword} disabled={resetLoading}>
    ...
    return (
      <div style={s.authPage}>
        <div style={s.authCard}>
          <div style={s.authLogo}>✦</div>
          <h1 style={s.authTitle}>TaskFlow</h1>
          <p style={s.authSub}>Enter your registered email to reset your password</p>
          <input style={s.input} placeholder="Email address" type="email"
            value={forgotEmail}
            onChange={e => setForgotEmail(e.target.value)}
            onKeyDown={e => e.key === "Enter" && handleForgotPassword()} />
          {forgotMsg && (
            <p style={{ ...s.authError, color: forgotMsg.includes("sent") ? "#22c55e" : "#ef4444" }}>
              {forgotMsg}
            </p>
          )}
          <button style={s.primaryBtn} onClick={handleForgotPassword} disabled={forgotLoading}>
            {forgotLoading ? "Sending..." : "Send Reset Link"}
          </button>
          <button style={{ ...s.cancelBtn, marginTop: "10px", width: "100%" }}
    ...
    return (
      <div style={s.authPage}>
        <div style={s.authCard}>
          ...
          {authError && (
            <p style={{ ...s.authError, color: authError.startsWith("Registered") ? "#22c55e" : "#ef4444" }}>
              {authError}
            </p>
          )}
          ...
          {screen === "login" && (
            <p style={{ textAlign: "center", marginTop: "12px", fontSize: "13px" }}>
              <button style={s.linkBtn}
                onClick={() => { setScreen("forgot"); setAuthError(""); }}>
                Forgot password?
              </button>
            </p>
          )}
...
const s = {
  ...
  authError:  { fontSize: "13px", marginBottom: "10px", textAlign: "center" },
  ...
  linkBtn:    { background: "none", border: "none", color: "#6366f1", cursor: "pointer", fontSize: "13px", textDecoration: "underline", padding: 0 },
